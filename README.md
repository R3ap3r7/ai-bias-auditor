...
A separate Flutter web front end now lives in `flutter_firebase_auditor/`. It uses the existing FastAPI service as the audit engine and Firebase Auth/Firestore for Google-owned audit-history storage. See `flutter_firebase_auditor/README.md` for setup, Firestore rules, and Firebase Hosting deployment.
...
When serving the Flutter web app from Firebase Hosting, also set `CORS_ALLOWED_ORIGINS` on Cloud Run to the Firebase Hosting origins. Cloud Run's free tier still requires a billing account on the Google Cloud project before `run.googleapis.com` and `cloudbuild.googleapis.com` can be enabled.
