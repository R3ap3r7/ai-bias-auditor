...
import '../dashboard/dashboard_screen.dart'; // For UserAvatarMenu
...
class _LandingScreenState extends State<LandingScreen> {
  ...
  Widget _buildNavLinks({bool isDrawer = false}) {
    final links = {
      'Features': _featuresKey,
      'How It Works': _howItWorksKey,
      'Datasets': _datasetsKey,
    };
    
    final items = links.entries.map((e) {
      return Padding(
        padding: isDrawer 
        ...
        child: InkWell(
          ...
          child: Text(e.key, style: AppTypography.titleMedium.copyWith(color: AppColors.textWhite)),
        ),
      );
    }).toList();
                    ...
                    if (isLoggedIn) {
                      return Row(
                        ...
                        children: [
                          TextButton(
                            onPressed: () => Navigator.pushNamed(context, '/dashboard'),
                            child: Text('Dashboard', style: AppTypography.titleMedium.copyWith(color: AppColors.textWhite)),
                          ),
                          const SizedBox(width: 16),
        ...
        slivers: [
          SliverToBoxAdapter(
            child: RepaintBoundary(
              child: _HeroSection(
                offset: _heroOffset,
                onDemoTap: () => _scrollTo(_datasetsKey),
              ),
            ),
          ),
...
class _MarqueeTickerState extends State<_MarqueeTicker> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    final items = _text.split(' · ').where((e) => e.isNotEmpty).toList();
    final rowChildren = items.map((e) => Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        _TickerItem(text: e),
        const SizedBox(width: 8),
        Text('·', style: AppTypography.bodyMedium.copyWith(color: AppColors.textMuted, fontSize: 16)),
        const SizedBox(width: 8),
      ],
    )).toList();
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: _isHovered ? const Color.fromRGBO(124, 58, 237, 0.12) : Colors.transparent,
          borderRadius: BorderRadius.circular(999),
        ),
        child: Text(
          widget.text,
          style: AppTypography.bodyMedium.copyWith(
            color: _isHovered ? AppColors.accentPrimary : AppColors.textMuted,
            fontSize: 16,
          ),
        ),
      ),
